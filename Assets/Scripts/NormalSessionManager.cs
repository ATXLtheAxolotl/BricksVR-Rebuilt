...
public class NormalSessionManager : MonoBehaviour
{
    ...
    public const string UpstreamErrorText = "Received an error status from the BricksVR datastore.\n\nPlease join the Discord at https://bricksvr.com/discord or wait a few minutes.";
    ...
    private IEnumerator Start()
    {
        ...
        _avatarManager = realtimeGameobject.GetComponent<RealtimeAvatarManager>();
    ...
    private void Update()
    {
        if (_realtime.connected && (OVRInput.GetUp(OVRInput.Button.Start, OVRInput.Controller.Touch) || Input.GetKeyUp(KeyCode.M)))
        {
            if (!_inGameMenuUp)
            {
                _inGameMenuUp = true;
    ...
    private IEnumerator JoinRoom()
    {
        ...
        messageManager.DisplayMessage("Status:", "Establishing connection...");

        if (_roomName.Length != 6 && _roomName.Length != 8)
        {
            messageManager.DisplayMessage("Error:", "Room not found. Enter a valid room code or create a new room.", true);
        ...
        {
            messageManager.DisplayMessage("Error:", versionSupportedResponse == null ? NetworkErrorText : "Your game is out of date! Please update on Steam or the Oculus store.", true);
            joiningBackButton.SetActive(true);
            _joiningCoroutine = null;
            yield break;
        }

        messageManager.DisplayMessage("Status:", "Fetching room metadata...");
        ...
        messageManager.DisplayMessage("Status:", "Joining room...");
        ...
        _realtime.Connect(_roomPrefix + _roomName);
    ...
    private void HandleBrickDownloadError()
    {
        ...
        {
            messageManager.DisplayMessage("Error:", UpstreamErrorText, true);
        }
        else
        {
            messageManager.DisplayMessage("Error:", NetworkErrorText, true);
        }
    ...
    {
        downloadBricksOnLoad.StartLoading(_roomPrefix + _roomName, messageManager);
    ...
    {
        GameObject brick4x2 = Realtime.Instantiate("4x2", new Vector3(0, -10, 0), Quaternion.identity);
        GameObject brick2x2 = Realtime.Instantiate("2x2", new Vector3(0, -20, 0), Quaternion.identity);
        GameObject brick1x4 = Realtime.Instantiate("1x4", new Vector3(0, -30, 0), Quaternion.identity);
    ...
    private void DidConnectToRoom(Realtime realtime)
    {
        ...
        GameObject localHead = realtimeGameobject.GetComponent<RealtimeAvatarManager>().localAvatar.head.gameObject;
    ...
    {
        float verticalOffset = _realtime.connected ? 0.2f : 1.4f;
        Vector3 gazeDirection = head.transform.forward;
        gazeDirection.y = 0f;
        Vector3 headPosition = head.transform.position;
        menuBoard.transform.position = headPosition + (gazeDirection.normalized * (_realtime.connected ? 2.8f : 10f));
